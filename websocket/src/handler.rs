...
use crate::game_action::{create_lobby, game_move, get_game_details, get_lobby_details, join_lobby, list_games, list_lobbies, quit_game, quit_lobby};
use crate::payload::{
    WebSocketAction::*,
    WebSocketPayload
};
...
pub(crate) async fn handle_websocket(websocket: WebSocket, state: Arc<WebSocketGameState>) {
    ...
    let mut receiver_task = tokio::spawn(async move {
        while let Some(Ok(message)) = stream.next().await {
            if handle_message(message, state.clone(), &mut sender).await.is_break() {
                break;
            }
        }
    });
